...
import { officeApi, usersApi } from "@/lib/api";
import { toast } from "@/components/ui/toast";
import { MapPin, Plus, Edit, Trash2, Users, Search } from "lucide-react";
...
export default function OfficeManagementPage() {
  ...
  const loadUsers = async () => {
    try {
      ...
      // Note: You may need to create a getAllUsers endpoint or use search
                ...
                <Card key={office.id}>
                  ...
                  <CardContent className="space-y-3">
                    <div className="text-sm space-y-1">
                      <div>
                        <strong>Location:</strong> {office.latitude.toFixed(6)}, {office.longitude.toFixed(6)}
                        <a
                          href={getLocationUrl(office.latitude, office.longitude)}
                          target="_blank"
                      ...
                      {(office.city || office.state || office.country) && (
                        <div>
                          <strong>Location:</strong> {[office.city, office.state, office.country].filter(Boolean).join(", ")}
                        </div>
                      )}
